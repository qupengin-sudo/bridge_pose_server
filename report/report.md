...
- 自動計數與計時：以有限狀態機管理動作階段轉換，當使用者完成一次完整的「抬臀→放下」循環即自動累計正確次數；若動作未達標準（未完全抬起即放下），則累計錯誤次數。同時支援使用者自訂訓練時間。

- 音效回饋：動作完成時播放正確音效，動作不合格時播放錯誤音效，讓使用者在運動中無需注視螢幕也能即時得知動作是否正確。

- 防抖機制：狀態轉換需維持一段時間（預設 0.8 秒）方可生效，避免因姿勢瞬間波動導致誤判。
...
- 小明打開電腦，在瀏覽器中輸入系統網址 `http://127.0.0.1:8000`，進入橋式健身輔助系統的首頁。頁面中央顯示「橋式健身輔助系統」標題與一個綠色的「開始運動」按鈕，右上角顯示「訪客」字樣。

- 他點擊右上角的「登入 / 註冊」按鈕，頁面切換至帳號管理頁，顯示「我要註冊」與「我要登入」兩個按鈕。小明點擊「我要登入」，畫面出現帳號與密碼輸入框，他輸入帳號密碼後點擊「登入」。若是第一次使用，他可以先點擊「我要註冊」建立帳號，註冊成功後會自動切換到登入表單。

- 登入成功後，右上角的「訪客」變為「你好，小明」，並出現「登出」和「查看所有紀錄」按鈕。頁面自動返回主頁。

- 小明點擊「開始運動」按鈕，按鈕文字變為「視訊啟動中...」且無法再次點擊。系統在伺服器端彈出一個深色主題的計時器選擇視窗，標題為「橋式訓練計時器」，內含分鐘與秒數的 Spinbox 選擇器。小明選擇 3 分鐘 0 秒，點擊「開始訓練」。

- 攝影機畫面隨即在一個標題為「Bridge Exercise」的 OpenCV 視窗中顯示，視窗大小約為螢幕高度的 70%，置中於主螢幕。畫面左上角有一個 230×310 像素的資訊面板，顯示「Stage: INIT」（灰色）、「OK:0 NG:0」、「Rate:0% (0/0)」以及「Time: --:-- (do pose to start)」。

- 小明躺在瑜珈墊上，將攝影機架設在能拍到全身側面的位置。他彎曲膝蓋，腳掌平放地面，雙手放在身體兩側。此時系統透過 MediaPipe 標記出他的肩膀、髖部、膝蓋、腳踝四個關節點，但因為臀部尚未抬起，膝-髖-肩角度小於 155°，狀態維持在 INIT。

- 小明緩緩抬起臀部，當膝-髖-肩角度持續達到 155° 以上且維持 0.8 秒後，系統狀態從 INIT 轉換至 STAGE1，畫面上的骨架線條與關節圓點變為綠色，資訊面板顯示「Stage: STAGE1」。倒數計時同時啟動，顯示「Time: 03:00」並開始遞減。

- 小明緩緩放下臀部，角度逐漸下降。當角度降至 131°–154° 的 TRANSITION 範圍且持續 0.8 秒後，狀態變為 TRANSITION（紅色）。角度繼續下降至 130° 以下並持續 0.8 秒後，狀態進入 STAGE2（橘色）。由於此次動作曾經到達 STAGE1（`reached_stage1 = True`），系統判定為一次正確的橋式動作，正確次數從 0 變為 1，同時播放 `correct.mp3` 音效。資訊面板更新為「OK:1 NG:0」、「Rate:100% (1/1)」。

- 小明持續重複動作。某一次他急於抬臀但未完全到位（角度僅達到 TRANSITION 範圍的 145°，未達 STAGE1 的 155°），隨即放下臀部。由於此次動作從 TRANSITION 直接進入 STAGE2 且 `reached_stage1 = False`，系統判定為一次錯誤動作，錯誤次數加 1，播放 `incorrect.mp3` 音效。資訊面板更新為「OK:12 NG:1」、「Rate:92% (12/13)」。

- 3 分鐘結束後（倒數計時最後 10 秒會以紅色閃爍提示），攝影機畫面自動關閉，彈出訓練結果對話框，顯示：標題「訓練完成！」、大字綠色數字「15」、「次橋式 / bridge reps」、紅色文字「錯誤次數：2」、「成功率：88.2% (15/17)」、「訓練時間 3 分鐘」。小明點擊「關閉」按鈕。

- 瀏覽器中的按鈕恢復為「開始運動」，同時彈出 alert 提示「運動結束！成績已為您自動存檔（正確 15 次，錯誤 2 次 / 180 秒）。」接著頁面自動跳轉至紀錄頁，上方顯示每日運動統計圖表（綠色正確次數長條、紅色錯誤次數長條、藍色成功率折線），下方顯示歷史紀錄列表。

訪客使用：

- 即使不登入，小明也可以直接點擊「開始運動」進行訓練。此時前端會先彈出 alert 提示「還未登入，我們會先紀錄起來。等待您登入後會一次送出。」

- 運動結束後，前端會將成績暫存於 JavaScript 變數 `pendingRecord` 中（包含 duration、count、wrong），並彈出提示「運動結束！正確 N 次，錯誤 M 次，共 S 秒。小提示：現在登入或註冊，系統會自動為您存檔喔！」

- 若小明此時登入，系統會自動檢測到 `pendingRecord` 不為 null，將暫存的紀錄透過 `/save_record` API 送至後端存入資料庫，並彈出提示「登入成功！已將您稍早的運動紀錄 (正確N次，錯誤M次) 自動存檔！」然後自動跳轉至紀錄頁。

# 第 2 章 相關知識及技術

本章介紹建構本系統所使用的技術與工具，包括姿態辨識框架、電腦視覺函式庫、後端框架、前端圖表函式庫、資料庫、桌面介面工具，以及音效播放函式庫。這些技術的介紹將為後續章節的系統架構和實作說明奠定基礎。
...
> 【此處需要圖片：系統架構圖，標示以下元件及其關係：瀏覽器（index.html）↔ FastAPI 伺服器（app.py）↔ 姿態偵測模組（bridge_detector.py）↔ 攝影機，FastAPI 伺服器 ↔ SQLite 資料庫（users.db），bridge_detector.py ↔ config.json、audio/、pose_landmarker_lite.task】
...
> 【此處需要圖片：使用者操作流程圖，從開啟瀏覽器到查看紀錄的完整流程】

### 3.2.2 橋式動作辨識

系統透過計算膝-髖-肩（Knee-Hip-Shoulder, KHS）角度來判斷橋式動作的階段。以髖部（Hip）為頂點，計算膝蓋（Knee）與肩膀（Shoulder）之間形成的角度。

> 【此處需要圖片：橋式動作角度示意圖，標示 Shoulder(C)、Hip(B)、Knee(A) 三點與角度 θ，以及使用者仰臥抬臀的姿勢】
...
根據 a_khs 的數值，`detect_bridge()` 函式判斷動作分為三個階段：

| 階段 | 角度範圍 | 判斷條件 | 顏色（BGR） | 說明 |
|:---:|:---:|:---:|:---:|:---|
| STAGE1（橋式完成） | 155°–180° | `STAGE1_MIN <= a_khs <= STAGE1_MAX` | (0, 255, 0) 綠色 | 臀部抬起，身體呈接近一直線 |
| TRANSITION（過渡） | 131°–154° | `STAGE2_MAX < a_khs < STAGE1_MIN` | (0, 0, 255) 紅色 | 臀部正在抬起或放下的中間狀態 |
| STAGE2（休息） | ≤ 130° | 不滿足上述兩個條件 | (0, 165, 255) 橘色 | 臀部放下，回到休息位置 |

上述角度範圍的數值由 `config.json` 中的 `STAGE1_MIN`（155）、`STAGE1_MAX`（180）、`STAGE2_MAX`（130）三個參數決定。TRANSITION 的範圍為 STAGE2_MAX 與 STAGE1_MIN 之間的區間（不含端點），即 131°至 154°，無需額外設定。

### 3.2.3 狀態機轉換

系統使用有限狀態機（Finite State Machine, FSM）管理動作階段的轉換，確保計數邏輯的正確性。狀態機以 Python 的 `Enum` 類別定義，包含四個狀態：

```python
class State(Enum):
    INIT       = auto()   # 等待第一個橋式動作
    STAGE1     = auto()   # 橋式完成（臀部抬起）
    TRANSITION = auto()   # 過渡階段（臀部部分抬起）
    STAGE2     = auto()   # 休息（臀部放下）
```

> 【此處需要圖片：狀態機轉換圖，四個圓形節點（INIT、STAGE1、TRANSITION、STAGE2），箭頭標示轉換條件（角度範圍 + 持續 HOLD_TIME 秒），STAGE1→STAGE2 與 TRANSITION(reached)→STAGE2 箭頭標示「correct +1 🔊」，TRANSITION(!reached)→STAGE2 箭頭標示「wrong +1 🔊」】
...
**INIT → STAGE1**：當角度偵測結果為 STAGE1 且持續達到 HOLD_TIME 秒。轉換時設定 `reached_stage1 = True`，若計時器尚未啟動則同時啟動計時器（`timer_started = True`，記錄 `timer_start_time = now`）。若角度偵測結果不為 STAGE1，清除 pending_state。

**STAGE1 → TRANSITION**：當角度偵測結果為 TRANSITION 且持續達到 HOLD_TIME 秒。

**STAGE1 → STAGE2**：當角度偵測結果為 STAGE2 且持續達到 HOLD_TIME 秒。轉換時 `bridge_count += 1`（正確計數），播放 `SFX_CORRECT.play()`，重設 `reached_stage1 = False`。

**TRANSITION → STAGE1**：當角度偵測結果為 STAGE1 且持續達到 HOLD_TIME 秒。轉換時設定 `reached_stage1 = True`。

**TRANSITION → STAGE2**：當角度偵測結果為 STAGE2 且持續達到 HOLD_TIME 秒。轉換時檢查 `reached_stage1`：
- 若為 True（本次循環曾到達 STAGE1）：`bridge_count += 1`，播放 `SFX_CORRECT.play()`。
- 若為 False（本次循環未曾到達 STAGE1，即使用者臀部未充分抬起即放下）：`wrong_count += 1`，播放 `SFX_INCORRECT.play()`。
- 兩種情況皆重設 `reached_stage1 = False`。

**STAGE2 → TRANSITION**：當角度偵測結果為 TRANSITION 且持續達到 HOLD_TIME 秒。開始新的動作循環。

**STAGE2 → STAGE1**：當角度偵測結果為 STAGE1 且持續達到 HOLD_TIME 秒。轉換時設定 `reached_stage1 = True`。
...
此機制的效果是：使用者的姿勢必須穩定在新狀態至少 0.8 秒才會被確認轉換。瞬間的抖動或晃動（持續時間 < 0.8 秒）不會觸發狀態轉換。HOLD_TIME 的值可透過 `config.json` 調整。
...
| 參數 | 型別 | 預設值 | 說明 |
|:---:|:---:|:---:|:---|
| DEVICE_ID | 整數 | 2 | 攝影機裝置編號。作業系統為每個攝影機裝置分配一個編號，通常 0 為內建鏡頭，1、2 為外接 USB 攝影機。可透過 `ls /dev/video*`（Linux）查看可用裝置。若開啟攝影機失敗，嘗試改為 0 或 1。 |
| STAGE1_MIN | 整數 | 155 | STAGE1（橋式完成）的最小膝-髖-肩角度（單位：度）。角度 ≥ 此值且 ≤ STAGE1_MAX 時判定為 STAGE1。降低此值可讓系統更容易判定為橋式完成（標準放寬）。 |
| STAGE1_MAX | 整數 | 180 | STAGE1（橋式完成）的最大膝-髖-肩角度。理論上人體關節角度不會超過 180°，此值通常不需調整。 |
| STAGE2_MAX | 整數 | 130 | STAGE2（休息）的最大角度。角度 ≤ 此值時判定為 STAGE2。降低此值可讓系統更不容易判定為休息（需要臀部放得更低才算休息）。 |
| HOLD_TIME | 浮點數 | 0.8 | 狀態轉換的防抖時間（單位：秒）。偵測到新狀態後需持續此時間方可確認轉換。增大此值可減少誤判但反應較慢；減小此值可提高靈敏度但可能增加誤判。建議範圍 0.3–1.5 秒。 |
...
若 `config.json` 中沒有 HOLD_TIME 欄位，程式會以 `CONFIG.get("HOLD_TIME", 1.0)` 取得預設值 1.0 秒。
...
1. **載入設定檔**：讀取 `config.json`，將 DEVICE_ID、STAGE1_MIN、STAGE1_MAX、STAGE2_MAX、HOLD_TIME 存為模組層級變數。
...
3. **定義 State 列舉**：`class State(Enum)` 定義四個狀態。

4. **定義顏色對照表**：`STATE_COLOR` 字典將每個狀態映射至 BGR 顏色元組，用於骨架繪製。INIT 為灰色 (200,200,200)、STAGE1 為綠色 (0,255,0)、TRANSITION 為紅色 (0,0,255)、STAGE2 為橘色 (0,165,255)。
...
回傳五個值：`in_bridge`（布林值，是否在 STAGE1）、`in_transition`（布林值，是否在 TRANSITION）、以及三個角度的浮點數值。若兩者皆為 False，則判定為 STAGE2。
...
設定所有狀態變數的初始值：`state = State.INIT`、`bridge_count = 0`、`wrong_count = 0`、`timer_started = False`、`finished = False`、`reached_stage1 = False`、`pending_state = None`、`pending_since = 0.0`、`timestamp = 0`。
...
d. **角度計算**：若有有效的關鍵點，呼叫 `detect_bridge()` 取得 `in_bridge`、`in_transition`、三個角度值。否則所有角度為 0.0。

e. **倒數檢查**：若計時已啟動且未結束，計算剩餘時間 `remaining = COUNTDOWN_SEC - (now - timer_start_time)`，若 ≤ 0 則設定 `finished = True`。

f. **偵測狀態判斷**：根據 `in_bridge` 和 `in_transition` 判斷 `detected` 為 STAGE1、TRANSITION 或 STAGE2。

g. **FSM 轉換**：若 `finished` 為 False，根據當前 `state` 與 `detected` 執行轉換邏輯（含防抖機制），詳見 3.2.3 節。
...
- **STAGE2_MAX（預設 130）**：若使用者臀部尚未完全放下即被判定為 STAGE2（橘色），可適當降低此值（例如改為 110 或 120）。相反，若使用者需要完全躺平才能觸發 STAGE2，可提高此值（例如改為 140）。

- **HOLD_TIME（預設 0.8）**：若系統因姿勢微幅晃動而頻繁切換狀態（例如在 STAGE1 與 TRANSITION 之間快速跳動），可增大此值（例如 1.0 或 1.5）。若系統反應太慢（使用者明顯已完成動作但計數延遲），可減小此值（例如 0.5 或 0.3）。

- **DEVICE_ID（預設 2）**：若攝影機無法開啟，嘗試改為 0 或 1。
